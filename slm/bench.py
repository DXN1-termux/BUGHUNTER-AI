...
def run_bench():
    ...
    if not cfg_path.exists():
        console.print("[red]config.toml not found - run slm setup[/red]")
        return
    ...
    if tps_p < TARGET_TOKS and fallback:
        console.print(
            f"[yellow]below target ({TARGET_TOKS} tok/s) - trying fallback[/yellow]")
    ...
    if chosen != primary:
        ...
        if new == original:
            console.print(
                f"[yellow]could not rewrite primary in {cfg_path}[/yellow]")
        else:
