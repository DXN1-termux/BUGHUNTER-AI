...
def _bench_one(model: str, n_threads: int = 6) -> float:
    ...
    if not bin_.exists():
        console.print([red]llama-bench missing — reinstall[/red]")
        return 0.0
    try:
        out = subprocess.check_output(
            [str(bin_), "-m", model, "-p", "0",
             "-n", "200", "-t", str(n_threads)],
            text=True, stderr=subprocess.STDOUT, timeout=180,
        )
    except subprocess.CalledProcessError as e:
        console.print([red]bench failed:[/red]\n{e.output[-500:]}")
        return 0.0
...
def run_bench():
    ...
    console.print([cyan]primary[/cyan]  {primary}")
    tps_p = _bench_one(primary, n_threads=n_threads)
    console.print(  → {tps_p:.1f} tok/s")
    chosen = primary
    if tps_p < TARGET_TOKS and fallback:
        console.print(
            [yellow]below target ({TARGET_TOKS} tok/s) — trying fallback[/yellow]")
        tps_f = _bench_one(fallback, n_threads=n_threads)
        console.print(  → {tps_f:.1f} tok/s")
        if tps_f > tps_p:
            chosen = fallback
    ...
    if chosen != primary:
        ...
        if new == original:
            console.print(
                [yellow]could not rewrite primary in {cfg_path}[/yellow]")
        else:
            cfg_path.write_text(new)
            console.print([green]selected {chosen}[/green]")
    else:
        console.print([green]keeping {chosen}[/green]")
