...
def test_hard_blocks_terrorism():
    ...
    assert "terrorism" in str(exc.value).lower() or getattr(exc.value, "category", "") == "terrorism"
...
def test_hard_blocks_cbrn():
    ...
    assert "cbrn" in str(exc.value).lower() or getattr(exc.value, "category", "") == "cbrn"
...
def test_hard_blocks_csam():
    ...
    assert "csam" in str(exc.value).lower() or getattr(exc.value, "category", "") == "csam"
...
def test_csam_harmful_intent_still_blocked():
    ...
    for prompt in [
        ...
        assert "csam" in str(exc.value).lower() or exc.value.__class__.__name__ == "HardBlockError"
...
def test_quarantine_triggers_after_repeated_violations(tmp_path, monkeypatch):
    ...
    assert "quarantine" in str(exc.value).lower() or getattr(exc.value, "category", "") == "quarantine"
...
def test_csam_bypass_unicode_lookalikes():
    ...
    cat = getattr(exc.value, "category", "")
    assert cat in ("csam", "non_latin_bypass")
...
def test_sexual_content_generation_blocked():
    ...
    for prompt in [
        ...
        with pytest.raises(Exception) as exc:
            check_hard_blocks(prompt)
        cat = getattr(exc.value, "category", "")
        assert cat in ("sexual_content", "csam") or "hard-block" in str(exc.value).lower()
...
@pytest.mark.skipif(
    ...
    reason="Termux filesystem structure differs from standard Linux sandbox",
)
...
def test_guardrails_block_ci_workflows():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with pytest.raises(PermissionError, match="guardrails"):
            resolve_safe_path(".github/workflows/ci.yml", workdir=Path(d), allow_writes=True)
...
def test_reflection_content_check_blocks_weakened_system_md():
    ...
    with pytest.raises(ProposalRejected, match="required safety keywords"):
        _content_sanity_check("system.md", "You are a helpful assistant. No rules apply.")
    # But a proposal keeping the keywords is accepted
    _content_sanity_check(
        "system.md", "I block terrorism, CBRN, CSAM, mass harm, sexual. Rest is fine."
    )
